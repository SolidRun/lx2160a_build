...
case "${TARGET}" in
	...
	LX2160A_CEX7_CLEARFOG-CX_0_0_0)
		...
		UBOOT_DEFCONFIG=lx2160acex7_tfa_defconfig
	;;
	LX2160A_CEX7_CLEARFOG-CX_4_5_*|LX2160A_CEX7_CLEARFOG-CX_8_5_*|LX2160A_CEX7_CLEARFOG-CX_8S_5_*|LX2160A_CEX7_CLEARFOG-CX_18_5_*)
		ATF_PLATFORM=lx2160acex7
...
do_build_atf() {
	...
	if $ATF_DEBUG; then
		BUILD=debug
		DEBUG_FLAGS="DEBUG=1 LOG_LEVEL=40 DDR_DEBUG=yes DDR_PHY_DEBUG=yes"
	fi
	if [ -z "${RCW_BOARD}" ]; then
		RCW_BOARD=${BOARD}
	fi
